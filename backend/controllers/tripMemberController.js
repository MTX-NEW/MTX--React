...
exports.getAllMembers = async (req, res) => {
  try {
    ...
    const whereCondition = searchQuery 
      ? {
          [Op.or]: [
            ...
            { ahcccs_id: { [Op.like]: `%${searchQuery}%` } },
            { phone: { [Op.like]: `%${searchQuery}%` } },
            // Using sequelize.where and sequelize.fn to search in related table
            where(
              fn('CONCAT', fn('LOWER', col('first_name')), ' ', fn('LOWER', col('last_name'))),
    ...
    const count = await TripMember.count({
