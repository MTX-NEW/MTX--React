...
router.get("/type/:typeId", async (req, res) => {
  try {
    ...
    console.error("Error in GET /type/:typeId:", error);
    res.status(500).json({ message: error.message });
  }
});

// Get permissions for a specific page
router.get("/by-page/:pageName", async (req, res) => {
  try {
    ...
    console.error("Error in GET /by-page/:pageName:", error);
    res.status(500).json({ message: error.message });
  }
});

// Update permissions for a page
router.post("/by-page/:pageName", async (req, res) => {
  ...
  try {
    ...
    await t.rollback();
    console.error("Error in POST /by-page/:pageName:", error);
    res.status(400).json({ message: error.message });
  }
});

// Validate if a user type has access to a page
router.get("/validate/:pageName/:typeId", async (req, res) => {
  try {
    const pageName = decodeURIComponent(req.params.pageName);
    const typeId = req.params.typeId;
    ...
    console.error("Error in GET /validate/:pageName/:typeId:", error);
    res.status(500).json({ message: error.message });
  }
});

// Remove a specific permission
router.delete("/page/:pageName/type/:typeId", async (req, res) => {
  try {
